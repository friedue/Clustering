...
# usage: ./computeHierarchicalClustering.R peakMatrix_ALL.tab euclidean ward new
...
#       [4] either "new" or  an existing distance.Robj

library(GMD)
args <- commandArgs(TRUE)

### defining filenames for saving
outFile_dist = paste("dist_", args[2], ".Robj", sep="")
outFile_hclust = paste("hclust_",  args[2],"_", args[3], ".Robj", sep="")
...
png(file=paste("dendrogram",args[2], args[3],"png", sep="."), width=1200, height=400)
plot(hh.dist, labels=FALSE, main=paste(args[2], args[3]))
dev.off()
